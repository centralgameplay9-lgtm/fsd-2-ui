          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card overflow-hidden mb-6">
            ...
            <div className="px-6 pb-6">
              <div className="flex flex-col sm:flex-row sm:items-end gap-4 -mt-16">
                <div className="w-24 h-24 rounded-lg bg-white dark:bg-stone-900 p-1 shadow-soft-lg shrink-0">
                  <img src={company.logo} alt={company.name} className="w-full h-full rounded-md object-cover" />
                </div>
                <div className="flex-1 pb-1">
                  <h1 className="text-2xl font-display font-medium">{company.name}</h1>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-5 pt-5 border-t border-stone-100 dark:border-stone-800/50">
                <div className="flex items-center gap-2 text-sm"><Users className="w-4 h-4 text-stone-400" /> {company.employees}</div>
                <div className="flex items-center gap-2 text-sm"><MapPin className="w-4 h-4 text-stone-400" /> {company.location}</div>
                <div className="flex items-center gap-2 text-sm"><Calendar className="w-4 h-4 text-stone-400" /> Founded {company.founded}</div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="card p-6">
                <h2 className="text-lg font-display font-medium mb-3">About {company.name}</h2>
                <p className="text-sm text-stone-600 dark:text-stone-300 leading-relaxed">{company.about}</p>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card p-6">
                ...
                <div className="space-y-4">
                  ...
                  {companyJobs.length === 0 && <p className="text-sm text-stone-500">No open positions at this time.</p>}
                </div>
              </motion.div>
              ...
              {company.gallery.length > 0 && (
                <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="card p-6">
                  <h2 className="text-lg font-display font-medium mb-4">Gallery</h2>
            ...
            <div className="space-y-4">
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="card p-5">
                ...
                <div className="space-y-2.5 text-sm">
                  <div className="flex justify-between"><span className="text-stone-500">Industry</span><span className="font-medium">{company.industry}</span></div>
                  <div className="flex justify-between"><span className="text-stone-500">Company Size</span><span className="font-medium">{company.size}</span></div>
                  <div className="flex justify-between"><span className="text-stone-500">Founded</span><span className="font-medium">{company.founded}</span></div>
                  <div className="flex justify-between"><span className="text-stone-500">Location</span><span className="font-medium">{company.location}</span></div>
                  <div className="flex justify-between"><span className="text-stone-500">Website</span><span className="font-medium text-primary-600">{company.website}</span></div>
                  <div className="flex justify-between"><span className="text-stone-500">Open Jobs</span><span className="font-medium">{company.openJobs}</span></div>
                </div>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="card p-5">
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div>
                    <div className="flex gap-0.5">{[1, 2, 3, 4, 5].map((s) => <Star key={s} className={cn('w-4 h-4', s <= Math.round(company.rating) ? 'fill-warning-500 text-warning-500' : 'text-stone-300 dark:text-stone-700')} />)}</div>
                    <p className="text-xs text-stone-500 mt-0.5">Based on 248 reviews</p>
                  </div>
                </div>
                <div className="space-y-2">
                  {[{ label: 'Culture', val: 4.8 }, { label: 'Salary', val: 4.5 }, { label: 'Work-life', val: 4.7 }, { label: 'Growth', val: 4.6 }].map((r) => (
                    <div key={r.label} className="flex items-center gap-2">
                      <span className="text-xs text-stone-500 w-16">{r.label}</span>
