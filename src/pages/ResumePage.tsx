  ...
  return (
    <DashboardLayout navItems={navItems} title="Resume Management" subtitle="Upload and manage your resumes" role={role}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card p-6 lg:col-span-2">
          ...
          <div
            ...
            className={cn(
              ...
              dragging ? 'border-primary-500 bg-primary-50/50 dark:bg-primary-900/20' : 'border-slate-300 dark:border-slate-700 hover:border-primary-400 hover:bg-slate-50 dark:hover:bg-slate-800/50'
            )}
          >
            ...
            <p className="text-xs text-slate-500 mt-1">or click to browse · PDF, DOCX, DOC (max 5MB)</p>
          ...
          {resume && (
            <div className="mt-5">
              ...
              <div className="flex items-center gap-3 p-4 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500">{resume.size} · Uploaded {resume.date}</p>
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => setPreviewOpen(true)} className="p-2 rounded-md hover:bg-white dark:hover:bg-slate-700 transition" title="Preview"><Eye className="w-4 h-4 text-slate-500" /></button>
                  <button onClick={() => toast('Resume downloaded!', 'success')} className="p-2 rounded-md hover:bg-white dark:hover:bg-slate-700 transition" title="Download"><Download className="w-4 h-4 text-slate-500" /></button>
                  <button onClick={() => { setResume(null); toast('Resume deleted', 'info'); }} className="p-2 rounded-md hover:bg-white dark:hover:bg-slate-700 transition" title="Delete"><Trash2 className="w-4 h-4 text-danger-500" /></button>
                </div>
              </div>
            </div>
          )}
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card p-5 mt-4">
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="flex items-center gap-3 p-3 rounded-md border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
              ...
              <button className="p-1.5 rounded-md hover:bg-slate-100 dark:hover:bg-slate-700 transition"><Download className="w-4 h-4 text-slate-500" /></button>
            </div>
          ))}
        </div>
      </motion.div>
      ...
      <Modal open={previewOpen} onClose={() => setPreviewOpen(false)} title="Resume Preview" className="max-w-2xl">
        <div className="aspect-[8.5/11] rounded-md bg-white border border-slate-200 dark:border-slate-800 p-8 overflow-y-auto max-h-[70vh]">
          <div className="text-center pb-4 border-b-2 border-primary-600">
            <h2 className="text-2xl font-bold text-slate-900">Jessica Williams</h2>
            <p className="text-sm text-slate-600 mt-1">Senior Frontend Engineer</p>
            <p className="text-xs text-slate-500 mt-2">San Francisco, CA · jessica.williams@email.com · jessicawilliams.dev</p>
          </div>
          <div className="mt-4">
            <h3 className="text-sm font-bold text-primary-700 uppercase tracking-wide mb-2">Summary</h3>
            <p className="text-xs text-slate-600 leading-relaxed">Frontend engineer with 6 years of experience building delightful, accessible web applications. Passionate about design systems, performance, and developer experience.</p>
          </div>
          <div className="mt-4">
            <h3 className="text-sm font-bold text-primary-700 uppercase tracking-wide mb-2">Experience</h3>
            <div className="space-y-3">
              <div>
                <p className="text-xs font-semibold text-slate-900">Frontend Engineer · Google</p>
                <p className="text-xs text-slate-500">2021 - Present</p>
                <p className="text-xs text-slate-600 mt-1">Building internal tools used by 10k+ Googlers. Led migration to React 18.</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-900">Frontend Engineer · Airbnb</p>
                <p className="text-xs text-slate-500">2018 - 2021</p>
                <p className="text-xs text-slate-600 mt-1">Developed booking flows and host dashboard features.</p>
              </div>
            </div>
          </div>
          <div className="mt-4">
            <h3 className="text-sm font-bold text-primary-700 uppercase tracking-wide mb-2">Education</h3>
            <p className="text-xs font-semibold text-slate-900">M.S. Computer Science, Stanford University</p>
          ...
          <div className="mt-4">
            ...
            <p className="text-xs text-slate-600">React, TypeScript, Next.js, Tailwind CSS, GraphQL, Framer Motion, Accessibility, Design Systems</p>
          </div>
        </div>
      </Modal>
    </DashboardLayout>
