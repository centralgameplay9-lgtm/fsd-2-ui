    ...
    <DashboardLayout navItems={navItems} title="Dashboard" subtitle="Welcome back, Jessica!" role={role}>
      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-6">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card p-5 lg:col-span-2">
          ...
          <p className="text-xs text-slate-500 mt-2">Add your portfolio and certifications to reach 100% and increase visibility to employers.</p>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="card p-5">
          ...
          <div className="space-y-2">
            <Link to="/jobs" className="flex items-center justify-between p-2.5 rounded-md hover:bg-slate-50 dark:hover:bg-slate-800 transition group">
            ...
            <Link to="/resume" className="flex items-center justify-between p-2.5 rounded-md hover:bg-slate-50 dark:hover:bg-slate-800 transition group">
              <span className="text-sm flex items-center gap-2"><FileText className="w-4 h-4 text-accent-500" /> Upload Resume</span>
              <ArrowRight className="w-4 h-4 text-slate-400 group-hover:translate-x-0.5 transition" />
            </Link>
            <Link to="/profile" className="flex items-center justify-between p-2.5 rounded-md hover:bg-slate-50 dark:hover:bg-slate-800 transition group">
            ...
            {applicationTimeline.map((item, i) => (
              <div key={item.id} className="flex gap-3">
                ...
                <div className="flex-1 pb-2">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">{item.company} · {item.step}</p>
                  <p className="text-xs text-slate-400 mt-0.5">{item.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="card p-5">
          ...
          <div className="space-y-3">
            {upcomingInterviews.map((iv) => (
              <div key={iv.id} className="flex items-start gap-3 p-3 rounded-md bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Based on your profile and applications</p>
          </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {recommendedJobs.map((job) => {
            ...
            return (
              <Link key={job.id} to={`/jobs/${job.id}`} className="p-3 rounded-md border border-slate-200 dark:border-slate-800 hover:border-primary-300 dark:hover:border-primary-700 hover:shadow-soft transition group">
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5 flex items-center gap-1"><MapPin className="w-3 h-3" /> {job.location}</p>
                  </div>
                </div>
                <div className="flex items-center justify-between mt-2.5 pt-2.5 border-t border-slate-100 dark:border-slate-800/50">
