      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-6">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card p-5 lg:col-span-2">
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-stone-500 mt-0.5">109 total applications received</p>
            </div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="card p-5">
          ...
          <div className="space-y-2">
            <Link to="/employer/post-job" className="flex items-center justify-between p-2.5 rounded-md hover:bg-stone-50 dark:hover:bg-stone-800 transition group">
            ...
            <Link to="/employer/applicants" className="flex items-center justify-between p-2.5 rounded-md hover:bg-stone-50 dark:hover:bg-stone-800 transition group">
              <span className="text-sm flex items-center gap-2"><Users className="w-4 h-4 text-accent-500" /> Review Applicants</span>
              <ArrowRight className="w-4 h-4 text-stone-400 group-hover:translate-x-0.5 transition" />
            </Link>
            <Link to="/employer/jobs" className="flex items-center justify-between p-2.5 rounded-md hover:bg-stone-50 dark:hover:bg-stone-800 transition group">
              <span className="text-sm flex items-center gap-2"><Briefcase className="w-4 h-4 text-success-500" /> Manage Jobs</span>
              <ArrowRight className="w-4 h-4 text-stone-400 group-hover:translate-x-0.5 transition" />
            </Link>
            <Link to="/employer/reports" className="flex items-center justify-between p-2.5 rounded-md hover:bg-stone-50 dark:hover:bg-stone-800 transition group">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="card p-5">
          ...
          <div className="space-y-3">
            {employerJobs.map((job) => (
              <div key={job.id} className="flex items-center gap-3 p-3 rounded-md border border-stone-200 dark:border-stone-800 hover:shadow-soft transition">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-stone-500">{job.applicants} applicants · {timeAgo(job.postedAt)}</p>
                </div>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="card p-5">
          ...
          <div className="space-y-3">
            {recentApplicants.map((app) => (
              <div key={app.id} className="flex items-center gap-3 p-3 rounded-md border border-stone-200 dark:border-stone-800 hover:shadow-soft transition">
                <img src={app.avatar} alt={app.name} className="w-10 h-10 rounded-full object-cover shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{app.name}</p>
                  <p className="text-xs text-stone-500">{app.experience} · {app.match}% match</p>
                </div>
