      ...
      <main className="flex-1">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
          <div className="mb-8">
            ...
            <p className="mt-2 text-slate-500">Discover great companies hiring on HireHub</p>
          </div>

          <div className="flex items-center gap-2 px-4 py-2.5 rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 mb-6 max-w-md">
            ...
            {filtered.map((c, i) => (
              <motion.div key={c.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                <Link to={`/companies/${c.id}`} className="card p-5 hover:shadow-soft-lg hover:border-primary-200 dark:hover:border-primary-800 transition group block">
                  ...
                  <p className="text-sm text-slate-500 mt-3 line-clamp-2">{c.about}</p>
                  <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100 dark:border-slate-800/50">
                    <span className="text-xs text-slate-500 flex items-center gap-1"><MapPin className="w-3 h-3" /> {c.location}</span>
